...
def main(mesh_name, init_benchmark_time):
    ...
    fortio_dir = os.path.join(root_dir, "benchmark", "fortio")
    run_fortio = os.path.join(root_dir, "benchmark", "fortio", "run-fortio.sh")
    run(f'{run_fortio} "{mesh_name}" "{fortio_dir}" "{init_benchmark_time}"')

    k6_dir = os.path.join(root_dir, "benchmark", "k6")
    run_k6 = os.path.join(root_dir, "benchmark", "k6", "run-tests.sh")
    run(f'{run_k6} "{mesh_name}" "{k6_dir}" "{init_benchmark_time}"')

    print("Waiting for test results...")
    time.sleep(30)

    end_time = int(time.time())
    metrics_dir = os.path.join(root_dir, "benchmark", "metrics")
    extract_script = os.path.join(root_dir, "benchmark", "metrics", "extract-prometheus-metrics.sh")
    run(f'{extract_script} "{start_time}" "{end_time}" "{mesh_name}" "{metrics_dir}" "{init_benchmark_time}"')
