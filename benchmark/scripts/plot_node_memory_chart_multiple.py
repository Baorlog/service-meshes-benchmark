...
def load_memory_metrics(mesh, timestamp):
    file_path = os.path.join(METRICS_DIR, timestamp, mesh, f"node_memory_{mesh}.json")
...
def plot_average_node_memory_chart(run_id, timestamps):
    ...
    for mesh in os.listdir(os.path.join(METRICS_DIR, timestamps[0])):
        ...
        for timestamp in timestamps:
            times, values = load_memory_metrics(mesh, timestamp)
        ...
        if merged_values:
            ...
            plt.plot(all_times[:len(avg_values)], avg_values, label=mesh, color="tab:blue")
            ...
            plt.legend()
            plt.tight_layout()
            out_path = os.path.join(OUTPUT_DIR, run_id, f"avg_node_memory_{mesh}.png")
    ...
    if mesh_values_accumulator:
        ...
        plt.legend()
        plt.tight_layout()
        out_path = os.path.join(OUTPUT_DIR, run_id, "avg_node_memory_chart.png")
        plt.savefig(out_path)
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python3 plot_node_memory_chart_multiple.py <run_id> <timestamp1> [timestamp2 ...]")
        sys.exit(1)

    run_id = sys.argv[1]
    timestamps = sys.argv[2:]
    plot_average_node_memory_chart(run_id, timestamps)
