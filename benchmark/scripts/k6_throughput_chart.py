...
def extract_k6_throughput(init_benchmark_times):
        ...
        for mesh in os.listdir(timestamp_dir):
            ...
            for idx, case_file in enumerate(K6_CASES):
                ...
                with open(file_path, "r") as f:
                    data = json.load(f)
                    rate = data.get("metrics", {}).get("http_reqs", {}).get("rate", 0)
                    mesh_case_data[f"case{idx+1}"][mesh].append(round(rate, 2))
...
def plot_line_chart(case_data, run_id):
    # Thêm marker style và line style để phân biệt đen trắng, nhưng vẫn giữ nguyên màu
...
def plot_bar_charts(case_data, run_id):
    for case, mesh_rates in case_data.items():
        ...
        bars = plt.bar(meshes, values, color=colors, edgecolor='black')  # Giữ màu, viền đen cho rõ

        plt.title(f"Throughput - K6 - {case}")
