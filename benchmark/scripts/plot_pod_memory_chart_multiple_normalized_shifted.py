...
def load_pod_memory_metrics(mesh, timestamp):
    file_path = os.path.join(METRICS_DIR, timestamp, mesh, f"pod_memory_{mesh}.json")
    ...
    for result in data["data"]["result"]:
        ...
        for v in values:
            rel_time = v[0] - t0
            usage_mb = float(v[1]) / (1024 * 1024)
            pod_data[pod_name].append((rel_time, usage_mb))

    return pod_data

def plot_pod_memory_chart_multiple(run_id, timestamps):
    ...
    for mesh in mesh_list:
        ...
        total_mem_by_time = defaultdict(list)

        for ts in timestamps:
            pod_data = load_pod_memory_metrics(mesh, ts)
            if not pod_data:
                continue
            ...
            for rel_t, total in run_total.items():
                total_mem_by_time[rel_t].append(total)

        offset = mesh_offsets[mesh]
        avg_total = {t + offset: sum(vals)/len(vals) for t, vals in total_mem_by_time.items()}
        ...
        out_path = os.path.join(OUTPUT_DIR, run_id, f"avg_pod_memory_{mesh}.png")
        fig.savefig(out_path)
        plt.close(fig)
        print(f"Saved pod-level memory chart: {out_path}")
    ...
    if mesh_total_avg:
        ...
        plt.title("Total Pod Memory Usage Per Mesh (Average of Runs, Shifted)")
        plt.xlabel("Time (s)")
        plt.ylabel("Memory Usage (MB)")
        plt.grid(True)
        plt.legend()
        plt.tight_layout()
        out_path = os.path.join(OUTPUT_DIR, run_id, "avg_pod_memory_total_chart_shifted.png")
        plt.savefig(out_path)
        print(f"Saved total memory comparison chart: {out_path}")

if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python3 plot_pod_memory_chart_multiple_normalized_shifted.py <run_id> <timestamp1> [timestamp2 ...]")
        sys.exit(1)

    run_id = sys.argv[1]
    timestamps = sys.argv[2:]
    plot_pod_memory_chart_multiple(run_id, timestamps)
